...
initiators["sing"] = function(input_q, player)
    ...
    if useCC then
        tmp_action_q:push(l_funcs.get_ja_action("Crooked Cards"))
    end
    ...
    for value in inputList:it() do
        if type(value) ~= "number" then
            if value ~= "cc" and rollCount < maxRolls then
                local buff = gd.rolls[value:lower()]
                if buff ~= nil then
                    hasValidRoll = true
                    local buff_name, spell = l_funcs.get_highest_tier_buff(player, buff, S(song_l))
                    song_l:append(spell)
        ...
        should_proc = (function(player, params)
            ...
            if params ~= nil and params.chatStr ~= nil then
                ...
                if leader_var ~= nil then
                    ...
                    if leader ~= nil then
                        if string.lower(speaker) ~= string.lower(leader) and (string.lower(speaker) ~= string.lower(player.instance.name)) then
