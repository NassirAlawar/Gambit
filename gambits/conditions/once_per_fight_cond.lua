...
function cond(uuid)
    local obj = {
        ...
        should_proc = (function(player, params)
            ...
            if last_target_index ~= nil then
                local last_mob = windower.ffxi.get_mob_by_index(last_target_index)
                if last_mob == nil or last_mob.hpp <= 0 then
                    params.g_bundle[uuid].last_target_index = nil
            ...
            return current_target_index ~= lmob.index and lmob['status'] == 1 and ltarget ~= nil and ltarget['claim_id'] ~= 0 and last_target_index ~= current_target_index, params
        end),
