...
-- debuffs: set of debuff names to check for
-- target_name: the name of the specific party member to check
function cond(debuffs, target_name)
    local obj = {
        ...
        should_proc = (function(player, params)
            for k, v in pairs(player.party) do
                if type(v) == 'table' then
                    local mob = v.mob
                    if mob ~= nil and mob.name == target_name then
